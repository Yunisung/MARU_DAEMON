#!/bin/sh
realtime_check=`ps -ef | grep -v "grep" | grep "charge_Settle_RECOVERY" | wc -l`
...
if [ "$realtime_check" == "0"  ]; then
    ...
    java $JVM_ARGS com.pgmate.dm.main.ChargeSettleRecovery
else
    echo "======== ChargeSettleRecovery Process Alive =======" >> /var/lib/jenkins/workspace/MARU_DAEMON/logs/root.txt
fi
