 ...
 *
 */
public class KcpDiffDownloadDAO extends DAO{

	private static Logger logger = LoggerFactory.getLogger( com.pgmate.dm.dao.KcpDiffDownloadDAO.class );

	public KcpDiffDownloadDAO() {
	}
	...
	public SharedMap<String,Object> getDiffUploadData(String seq){
		if(seq == null){
			return new SharedMap<String,Object>();
		} else {
			String key = "PG_MCHT_DIFF_UPLOAD" + seq;
			if (Cache.map.containsKey(key)) {
				return Cache.map.getUnchecked(key);
			} else {
