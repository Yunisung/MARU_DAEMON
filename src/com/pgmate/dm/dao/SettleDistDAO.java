 ...
 *
 */
public class SettleDistDAO extends SettleDAO implements ImplSettle {

	private static Logger logger = LoggerFactory.getLogger( com.pgmate.dm.dao.SettleDistDAO.class );

	public SettleDistDAO() {
	...
	public List<SharedMap<String,Object>> getSettleList(String stlDay){
		...
		String q = "SELECT A.distId as memberId,startDay,endDay,stlDay,payAmt,payCnt,payFee,payVat,rfdAmt,rfdCnt,rfdFee,rfdVat,stlFee,stlDiffFee,stlFee + stlDiffFee AS stlAmt,bankCd,bankName,account,accntHolder, '' as taxId FROM ( "
				...
				+"  FROM VW_TRX_CAP_LIST where stlDistDay between '"+month+"01' and '"+month+"31' and IFNULL(serviceType, '') != '월세앱' and stlDistId = '' group by distId, stlDistDay "
