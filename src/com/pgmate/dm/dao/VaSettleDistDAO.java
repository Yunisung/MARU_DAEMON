 ...
 *
 */
public class VaSettleDistDAO extends SettleVactDAO implements ImplVaSettle {

	private static Logger logger = LoggerFactory.getLogger( com.pgmate.dm.dao.VaSettleDistDAO.class );

	public VaSettleDistDAO() {
		super.setDebug(false);
	}
	
	public List<SharedMap<String,Object>> getSettleList(String stlDay){
		String month = stlDay.substring(0, 6);
		String q = "SELECT A.distId as memberId,A.startDay,A.endDay,A.stlDay,A.payAmt,A.payCnt,A.payFee,A.withdrawAmt,A.withdrawCnt,A.withdrawFee,A.stlFee,B.bankCd,B.bankName,B.account,B.accntHolder FROM ( "
				...
				+"  ,SUM(IF(trxType ='출금', stlDistFee,0)) withdrawFee "
				+"  ,SUM(stlDistFee) stlFee "		
				+"  FROM VW_VA_TRX WHERE stlDistDay BETWEEN '"+month+"01' and '"+month+"31' AND stlDistId = '' group BY distId, stlDistDay "
				+"  ) A, VA_MAM_DIST B WHERE A.distId = B.distId ";
	...
	public void setSettleToIdx(String stlId,String stlDay,String distId){
		String q = "INSERT INTO VA_SETTLE_IDX  "
				...
				+"  FROM VW_VA_TRX where stlDistDay ='"+stlDay+"' and distId = '"+distId+"'";
		logger.info("set VA_SETTLE_IDX : {}",super.update(q));
		super.initRecord();
	}
	
	public void updateTrx(String stlId){
		String q = "UPDATE VA_TRX_DTL SET stlDistId='"+stlId+"' "
