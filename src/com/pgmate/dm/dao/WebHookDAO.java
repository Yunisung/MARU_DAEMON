...
public class WebHookDAO extends DAO {
	...
	public List<SharedMap<String, Object>> getPayList() {
		String q = "SELECT E.* FROM ( "
				...
				+ "JOIN "
				+ "(SELECT C.mchtId,C.trxId,C.van,C.vanId,C.tmnId,concat(C.regDay, C.regTime) AS trxDate,C.regDay AS trxDay,C.trackId,C.vanTrxId,C.authCd,C.cardType,C.issuer,C.acquirer,C.bin,C.last4,C.installment,C.amount,C.regDate,C.rentId, "
...
//				+ "WHERE F.status = '전송실패' AND F.retry < 10 ORDER BY E.trxDate ASC ";
	...
	public List<SharedMap<String, Object>> getRfdList() {
		String q = "SELECT E.* FROM ( "
				...
				+ "(SELECT * FROM PG_MCHT_WEBHOOK WHERE trxType = 'CARD' AND STATUS = 'Y') A "
				+ "JOIN "
				+ "(SELECT C.mchtId,C.trxId,C.van,C.vanId,C.tmnId,concat(C.regDay, C.regTime) AS trxDate,C.regDay AS trxDay,C.trackId,C.vanTrxId,C.authCd,D.cardType,C.issuer,C.acquirer,C.bin,C.last4,D.installment,ABS(C.rfdAmount) AS amount,C.rootTrxId,C.regDate, "
