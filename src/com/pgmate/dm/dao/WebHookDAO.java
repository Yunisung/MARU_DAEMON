...
public class WebHookDAO extends DAO {
	...
	public List<SharedMap<String, Object>> getPayRetryList() {
		String q = "SELECT E.* FROM ( "
				...
				+ "(SELECT * FROM PG_MCHT_WEBHOOK WHERE trxType = 'CARD' AND STATUS = 'Y') A "
				+ "JOIN "
				+ "(SELECT C.mchtId, CONCAT(C.trxId, '_') as trxId,C.van,C.vanId,C.tmnId,concat(C.reqDay, C.regTime) AS trxDate,C.regDay AS trxDay,C.trackId,C.vanTrxId,C.authCd,C.cardType,C.issuer,C.acquirer,C.bin,C.last4,C.installment,C.amount,C.regDate,C.rentId, "
				+ " D.distId, D.agencyId, D.salesId "
				...
				+ "WHERE E.trxId='T240203348221_' AND F.status = '전송실패' AND F.retry < 10 ORDER BY E.trxDate ASC ";
