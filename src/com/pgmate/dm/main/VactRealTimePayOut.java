 ...
 *
 */
public class VactRealTimePayOut {
	private static Logger logger = LoggerFactory.getLogger( com.pgmate.dm.main.VactRealTimePayOut.class );
	...
	public static void main(String[] args){
		new VactRealTimePayOut();
	}
	
	public VactRealTimePayOut() {
		logger.info("==================================================");
		logger.info("VactRealTimePayOut Strart");
		smsGw = new SmsGw();
		
		configSetting();
		realTimePayOut();
		logger.info("VactRealTimePayOut End");
		logger.info("==================================================");
	}
	...
	public void realTimePayOut(){
		...
		try {
			...
			if(currentTime > firmEndTime || currentTime < firmStartTime) {
				logger.info("- -- --- ---- ---- ---- 가상계좌 실시간 출금 서비스 가능한 시간이 아닙니다. ---- ---- ---- --- -- -");
				return;
			}
			...
			logger.info("가상계좌 trxRealTimePayOut COUNT : {}", getRealTimePayList.size());
			
			if(getRealTimePayList.size() > 0) {
				logger.info("==================================================");
				logger.info("가상계좌 PG_REALTIME_PAYOUT INSERT START");
				...
				for(SharedMap<String,Object> data : getRealTimePayList){
					...
					SharedMap<String,Object> vactTrxMap	= dao.getVactTrx(data.getString("trxId"));
					...
					if("0".equals(data.getString("trxType"))) {
						payOutData.put("stlFee"			, vactTrxMap.getLong("stlFee"));
						payOutData.put("stlFeeVat"		, vactTrxMap.getLong("stlFeeVat"));
						payOutData.put("stlAmount"		, vactTrxMap.getLong("stlAmount"));
						
						payOutData.put("payOutFee"		, mchtMngVactMap.getLong("payOutFee"));
						...
						payOutData.put("payOutFee"		, mchtMngVactMap.getLong("payOutFee") * -1);
						payOutData.put("payOutFeeVat"	, calcVat(payOutData.getLong("payOutFee")));
						payOutData.put("payOutAmount"	, payOutData.getLong("stlAmount") - payOutData.getLong("payOutFee") - payOutData.getLong("payOutFeeVat"));
						...
						logger.info("가상계좌 실시간정산 취소거래 건 처리 : [{}]", data.getString("trxId"));
					}
				...
				logger.info("가상계좌 PG_REALTIME_PAYOUT INSERT END");
				logger.info("==================================================");
			}
			
			List<SharedMap<String,Object>> getPayOutList = dao.getPayOutList(payType);
			logger.info("가상계좌 realTimePayOut COUNT : {}", getPayOutList.size());
			
			if(getPayOutList.size() > 0) {
				logger.info("==================================================");
				logger.info("가상계좌 실시간 출금 START");
				...
				for(SharedMap<String,Object> data : getPayOutList){
					...
					if(data.getLong("payOutAmount") > 0){
						if("0".equals(data.getString("sendCnt"))) {
							logger.info("가상계좌 실시간 출금 : [{}][{}]", data.getString("trxId"), data.getString("sendCnt"));
							...
							if(!firmBean.resultCd.equals("0000") ) {
								msgBody = "가상계좌 실시간 출금 실패. trxId : [" + data.getString("trxId") + "], trackId : [" + data.getString("trackId") + "]";
								logger.info(msgBody);
								errFlag = true;
							}else {
								sendCheck = "Y";
								msgBody = "가상계좌 실시간 출금 성공. trxId : [" + data.getString("trxId") + "], trackId : [" + data.getString("trackId") + "]";
								logger.info(msgBody);
							}
						}else {
							String orgSeq = "";
							...
							logger.info("가상계좌 실시간 출금 결과확인 : [{}][{}][{}]", data.getString("trxId"), data.getString("sendCnt"), orgSeq);
							...
							if(!firmBean.resultCd.equals("0000") ) {
								msgBody = "가상계좌 결과확인 실패. trxId : [" + data.getString("trxId") + "], resultCd : [" + firmBean.resultCd + "], resultMsg : [" + firmBean.resultMsg + "]";
								logger.info(msgBody);
								errFlag = true;
							}else {
								sendCheck = "Y";
								msgBody = "가상계좌 결과확인 성공. trxId : [" + data.getString("trxId") + "], resultCd : [" + firmBean.resultCd + "], resultMsg : [" + firmBean.resultMsg + "]";
								logger.info(msgBody);
							}
						}
					} else {
						logger.info("가상계좌 실시간 출금 0원이하 거래건 : [{}][{}][{}]", data.getString("trxId"), data.getString("sendCnt"), data.getLong("payOutAmount"));
						
						sendCheck = "Y";
						firmBean.resultCd = "0000";
						firmBean.resultMsg = "가상계좌 0원이하 거래건";
					}
					...
					if(!dao.updatePayOutRes(data.getString("trxId"), CommonUtil.getCurrentDate("yyyyMMdd"), CommonUtil.getCurrentDate("HHmmss"), mchtTaxMap.getString("bankCd"), 
						...
						msgBody = "가상계좌 PG_REALTIME_PAYOUT UPDATE 실패. 확인요망 [" + data.getString("trxId") + "]";
					...
					if(errFlag) {
						...
						if(!"".equals(sndCnt)) {
							...
							if(errCnt == 3) {
								msgBody = "가상계좌 실시간 출금 3회 실패. 확인요망 [" + data.getString("trxId") + "][" + firmBean.resultMsg + "]";
				...
				logger.info("가상계좌 실시간 출금 END");
				logger.info("==================================================");
			}
		} catch(Exception e) {
			logger.error(e.getMessage(), e);
			
			msgBody = "가상계좌 실시간 출금 오류발생. 확인요망 [" + e.getMessage() + "]";
			smsGw.sendMessage("0", "4", msgBody);
		}
	}
